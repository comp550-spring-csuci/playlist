...
const Songs = () => {
  ...
  const {
    ...
    } = usePaginatedData(searchAppleMusicSongs, 50);
...
const Songs = () => {
  ...
  const {
    ...
    hasMore,
    fetchData: fetchMoreSongs,
  } = usePaginatedData(searchAppleMusicSongs, 25);
    ...
    <SafeAreaView style={styles.safeAreaContainer}>
      ...
      <View style={styles.searchContainer}>
        <TextInput
          ...
          style={styles.searchInput}
          onSubmitEditing={() => fetchMoreSongs(true)}
        />
      </View>
            ...
            >
              <View style={{ alignItems: "center" }}>
                <Image
                  ...
                  style={{ width: 100, height: 100, borderRadius: 8 }}
                />
                <Text style={{ maxWidth: 100, textAlign: "center", marginTop: 5 }} numberOfLines={1}>
                  {item.name}
                </Text>
              </View>
            </TouchableOpacity>
      ...
      {filteredSongs.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.songCard}>
              <Image
                source={{ uri: item.attributes.artwork.url.replace('{w}x{h}bb', '100x100bb') }}
                style={styles.songImage}
              />
              <View style={styles.songDetails}>
                <Text style={styles.songTitle}>
      ...
      <Modal transparent={true} visible={modalVisible} animationType="fade" onRequestClose={closeModal}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>ADD TO</Text>
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                router.push({
                  pathname: "/playlist",
                  params: {
                    songToAdd: JSON.stringify(selectedSong),
                  },
                });
              }}
            >
      ...
      <Modal transparent={true} visible={newPlaylistModalVisible} animationType="slide" onRequestClose={() => setNewPlaylistModalVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>Create New Playlist</Text>
            <TextInput
